...
import 'package:url_launcher/url_launcher.dart';
import 'package:flutter/foundation.dart' show kIsWeb;
import 'package:fc_native_video_thumbnail/fc_native_video_thumbnail.dart'; // NEW IMPORT
import 'package:path_provider/path_provider.dart'; // NEW IMPORT
import 'package:path/path.dart' as p; // NEW IMPORT for path manipulation
import 'package:chatview/src/widgets/custom_video_player.dart';
import 'package:chatview/src/values/typedefs.dart';
import 'package:chatview/src/models/config_models/message_configuration.dart';
...
class _VideoMessageViewState extends State<VideoMessageView> {
  Uint8List? _thumbnailData; // Changed to Uint8List
  bool _isLoadingThumbnail = true;
  final String _fallbackThumbnailAsset = 'assets/default_video_thumbnail.png'; // Ensure this asset exists

  double _videoAspectRatio = 16 / 9; // Default, will be updated by generated thumbnail

  // Initialize the fc_native_video_thumbnail plugin
  ...
  @override
  void didUpdateWidget(covariant VideoMessageView oldWidget) {
    super.didUpdateWidget(oldWidget);
    // If the message (and thus video URL/path) changes, regenerate thumbnail
    if (widget.message.message != oldWidget.message.message) {
      _thumbnailData = null; // Clear old thumbnail
      _videoAspectRatio = 16 / 9; // Reset aspect ratio
      _generateThumbnail();
    }
  }
  ...
  Future<void> _generateThumbnail() async {
    ...
    final String pathOrUrl = widget.message.message;
    print('offline-chatview: Attempting to generate thumbnail for: $pathOrUrl');

    if (kIsWeb || _isWebUrl(pathOrUrl)) { // fc_native_video_thumbnail doesn't support web URLs directly
      print('offline-chatview: Path is a web URL or running on web, skipping client-side thumbnail generation.');
      if (mounted) {
        setState(() {
          _isLoadingThumbnail = false;
          _thumbnailData = null;
          _videoAspectRatio = 16 / 9; // Fallback for web URLs or web platform
        });
      }
    } else {
      final String actualFilePath = _getCleanLocalPath(pathOrUrl);
      final File file = File(actualFilePath);

      if (await file.exists()) {
        print('offline-chatview: Video file exists at: $actualFilePath');
        try {
          // 1. Get a temporary directory to save the thumbnail
          final tempDir = await getTemporaryDirectory();
          final String fileName = p.basenameWithoutExtension(actualFilePath) + '_thumb.jpeg';
          final String destFile = p.join(tempDir.path, fileName);

          // 2. Generate the thumbnail and save it to the temporary file
          final bool thumbnailGenerated = await _thumbnailPlugin.getVideoThumbnail(
            srcFile: actualFilePath,
            destFile: destFile,
            width: 400, // Request larger for better quality, AspectRatio will control display
            height: 400,
            quality: 75,
            format: 'jpeg',
          );
          ...
          if (thumbnailGenerated) {
            print("offline-chatview: SUCCESS: Thumbnail generated and saved to: $destFile");
            // 3. Read the generated thumbnail file into Uint8List
            final File thumbnailFile = File(destFile);
            if (await thumbnailFile.exists()) {
              final Uint8List thumbnailBytes = await thumbnailFile.readAsBytes();

              // 4. Load the bytes to get actual dimensions for aspect ratio
              final Image image = Image.memory(thumbnailBytes);
              final Completer<ui.Image> completer = Completer<ui.Image>();
              image.image.resolve(const ImageConfiguration()).addListener(
                ImageStreamListener((ImageInfo info, bool _) {
                  completer.complete(info.image);
              ...
              if (actualHeight > 0) {
                _videoAspectRatio = actualWidth / actualHeight;
                print('offline-chatview: Calculated thumbnail aspect ratio: $_videoAspectRatio');
              } else {
                print('offline-chatview: Thumbnail height is 0, using default aspect ratio.');
              ...
              // Optionally, delete the temporary thumbnail file after it's loaded into memory
              // await thumbnailFile.delete();
            } else {
              print('offline-chatview: ERROR: Thumbnail file not found after generation: $destFile');
              if (mounted) {
                setState(() {
                  _isLoadingThumbnail = false;
                  _thumbnailData = null;
            ...
            print("offline-chatview: ERROR: fc_native_video_thumbnail returned FALSE for path: $actualFilePath");
            if (mounted) {
              setState(() {
                _isLoadingThumbnail = false;
                _thumbnailData = null;
          ...
          print("offline-chatview: CRITICAL ERROR generating thumbnail or getting dimensions: $e");
          print("Stack trace: $st");
          if (mounted) {
            setState(() {
              _isLoadingThumbnail = false;
        ...
        print('offline-chatview: ERROR: Video file does NOT exist at: $actualFilePath');
        if (mounted) {
          setState(() {
            _isLoadingThumbnail = false;
            _thumbnailData = null;
  ...
  void _onVideoTap(BuildContext context, String pathOrUrl) async {
    ...
    if (widget.customVideoPlayerBuilder != null) {
      print('offline-chatview: Using custom video player builder.');
      Navigator.push(
        context,
        MaterialPageRoute(
          builder: (context) => widget.customVideoPlayerBuilder!(pathOrUrl),
      ...
      print('offline-chatview: Opened video with custom builder.');
    } else {
      print('offline-chatview: Using default in-app video player (CustomVideoPlayer).');
      Navigator.push(
        context,
        MaterialPageRoute(
          builder: (context) => Scaffold(
      ...
      print('offline-chatview: Opened video with default in-app player.');
    }
    print('--- Video Tap Debug End ---');
  }
  ...
  Widget build(BuildContext context) {
    ...
    if (_isWebUrl(pathOrUrl)) {
      ...
      displayFileName = p.basename(_getCleanLocalPath(pathOrUrl)); // Use p.basename for file name
      displayIcon = Icons.video_file;
    }
                ...
                child: _isLoadingThumbnail
                    ...
                    : _thumbnailData != null
                        ? Stack(
                            alignment: Alignment.center,
                            children: [
                              Image.memory( // Display Uint8List thumbnail
                                _thumbnailData!,
                                width: double.infinity,
                                height: double.infinity,
                                fit: BoxFit.cover, // Use cover to fill the aspect ratio box
                                errorBuilder: (context, error, stackTrace) => Container(
                                  color: Colors.grey[200],
                                  child: Center(
                                    child: Icon(displayIcon, color: iconColor, size: 40),
                        ...
                        : Stack( // Fallback if thumbnail generation fails or is not applicable (e.g., web URL)
                            alignment: Alignment.center,
                            children: [
                              // You might want a more generic placeholder if _fallbackThumbnailAsset isn't always available
                              Container(
                                color: Colors.grey[200],
                                width: double.infinity,
                                height: double.infinity,
              ...
              child: Column(
                ...
                children: [
                  ...
                  _isWebUrl(pathOrUrl)
                      ...
                      : FutureBuilder<bool>(
                          future: File(_getCleanLocalPath(pathOrUrl)).exists(), // Use _getCleanLocalPath here
                          builder: (context, snapshot) {
                            if (snapshot.connectionState == ConnectionState.done) {
                              if (snapshot.hasData && snapshot.data == true) {
                                final file = File(_getCleanLocalPath(pathOrUrl)); // Use _getCleanLocalPath here
                                return Text(
                                  'Video exists (${(file.lengthSync() / 1024).toStringAsFixed(2)} KB)',
                                  style: const TextStyle(fontSize: 10, color: Colors.black54),
                                  maxLines: 1,
